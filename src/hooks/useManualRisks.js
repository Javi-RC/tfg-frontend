...
export function useManualRisks(projectId) {
  ...
  const loadManualRisks = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.error || err.message || i18n.t('risks.errors.loadFailed');
  ...
  const addRisk = useCallback(
    async (riskData) => {
      ...
      try {
        ...
        const errorMessage =
          err.response?.data?.error || err.message || i18n.t('risks.errors.addFailed');
        setError(errorMessage);
        return null;
      } finally {
        setLoading(false);
  ...
  const updateRisk = useCallback(
    async (riskId, updateData) => {
      ...
      try {
        ...
        setManualRisks((prev) => prev.map((risk) => (risk._id === riskId ? updatedRisk : risk)));

        return updatedRisk;
      } catch (err) {
        const errorMessage =
          err.response?.data?.error || err.message || i18n.t('risks.errors.updateFailed');
        setError(errorMessage);
        return null;
      } finally {
        setLoading(false);
  ...
  const deleteRisk = useCallback(
    async (riskId) => {
      ...
      try {
        ...
        const errorMessage =
          err.response?.data?.error || err.message || i18n.t('risks.errors.deleteFailed');
        setError(errorMessage);
        return false;
      } finally {
        setLoading(false);
  ...
  const repredictRisks = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.error || err.message || i18n.t('risks.errors.rePredictFailed');
      setError(errorMessage);
      return false;
    } finally {
      setLoading(false);
